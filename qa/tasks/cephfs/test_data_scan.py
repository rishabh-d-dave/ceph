...
class SimpleWorkload(Workload):
    ...
    def validate(self):
        self._mount.run_shell(["ls", "subdir"], omit_sudo=True)
...
class SymlinkWorkload(Workload):
    ...
    def validate(self):
        self._mount.run_shell(["ls", "symdir"], omit_sudo=True)
...
class TestDataScan(CephFSTestCase):
    ...
    def test_fragmented_injection(self):
        ...
        out = self.mount_a.run_shell_payload(f"cat subdir/{victim_dentry}", omit_sudo=True).stdout.getvalue().strip()
