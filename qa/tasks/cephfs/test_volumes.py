...
class TestVolumesHelper(CephFSTestCase):
    ...
    def _wait_for_clone_to_be_pending(self, clone, clone_group=None,
        ...
        self.__check_clone_state(states, clone, clone_group, timo, sleep)
...
class TestCloneProgressReporter(TestVolumesHelper):
    ...
    def filter_out_gre_from_pevs(self, progress_events):
        ...
        for k, v in progress_events.items():
            if 'global recovery event' in v['message'].lower():
                gre_key = k
                # Don't use "break". Usually there is only one "Global Recovery
    ...
    def _test_progress_bar_for_1_clone_diff_groups(self):
        '''
        ...
        self.run_ceph_cmd(f'fs subvolume snapshot clone {v} {group} {sv} {ss} {c}')
    ...
    def get_both_progress_fractions_and_onpen_count(self):
        ...
        bar.
        '''
        with safe_while(tries=20, sleep=1) as proceed:
            while proceed():
                o = self.get_ceph_cmd_stdout('status --format json-pretty')
                o = json.loads(o)
                pevs = o['progress_events']
