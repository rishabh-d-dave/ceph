 ...
 *
 * ng = new grant, new mds cap grant, shorter name makes this code more
 * succinct.
 */
...
{
  vector<bool> were_caps_merged;

  for (auto& ng : newcaps.grants) {
      bool is_merged = merge_one_cap_grant(ng);
      were_caps_merged.push_back(is_merged);
  }

  // if even one cap was merged, return true so that it can be added to
  // entity's keyring.
  for (auto was_cap_merged : were_caps_merged) {
    if (was_cap_merged) {
      return true;
    }
  }
  // Since above loop finished running, this means new cap are already present
  // in entity's keyring. Therefore, return false.
  return false;
}
