...
def get_statistics_for_path(given_path, fsh, op, should_cancel):
    ...
    '''
    def get_stats(given_path, stats):
        rfiles_old, rsubdirs_old, rbytes_old, same_thrice = 0, 0, 0, 0

        for i in range(1, 5):
            rfiles_new = fsh.getxattr(given_path, 'ceph.dir.rfiles')
            rbytes_new = fsh.getxattr(given_path, 'ceph.dir.rbytes')
            rsubdirs_new = fsh.getxattr(given_path, 'ceph.dir.rsubdirs')

            if rfiles_new == rfiles_old and rsubdirs_new == rsubdirs_old and \
                rbytes_new == rbytes_old:
                same_thrice += 1
        ...
        stats.rfiles_t = get_num_h(int(rfiles_new))
        stats.rsubdirs_t = get_num_h(int(rsubdirs_new))
        stats.rbytes_t = get_size_h(int(rbytes_new))
        return stats

    stats = get_stats(given_path, Stats(op=op))
